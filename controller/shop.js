...
exports.displayProduct = async (req, res, next) => {
  ...
  setTimeout((y) => {
    for (const i of coodiants) {
      ...
      L = Math.sqrt(
        Math.pow(lat1 - latCurrent, 2) + Math.pow(log2 - logCurrent, 2)
      );
      console.log('1');
      if (L < closedShop) {
        ORANGE = {
    ...
    prices.forEach((price) => {
      // console.log("3")
      if (price.new != price.old && price.new < CHEAP)
        (CHEAP = price.new), (GREEN = price);
      else if (price.new == price.old && price.old < CHEAP)
        (CHEAP = price.old), (GREEN = price);
    });
...
exports.deleteProduct = async (req, res, next) => {
  ...
  const updatedshops = product.shops.filter(
    (shop) => shop.sellerId != seller._id
  );
  ...
  if (product.shops.length == 0) {
    const deletedProductFromProducts = await Product.deleteOne({
      _id: productId,
    });
...
exports.updateProduct = async (req, res, next) => {
  ...
  const user = req.userr;
  const seller = await Seller.findOne({ infoUser: user._id });
  const { colors, sizes, price, quantity } = req.body;
  product.shops.forEach((shop) => {
